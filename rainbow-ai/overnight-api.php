...
if ($action === 'status') {
    ...
    }, array_values(array_filter($queue, 'is_array')));
...
if ($action === 'cancel') {
    ...
    foreach ($queue as &$job) {
        if (!is_array($job) || (string)($job['job_id'] ?? '') !== $jobId) continue;
        if (in_array((string)($job['status'] ?? ''), ['completed','cancelled'], true)) break;
    ...
    unset($job);
    if (!$found) rainbow_json(['ok'=>false,'code'=>'job_not_found_or_final'],404);
    rainbow_atomic_json_write($queueFile, $queue);
