...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($verifyToken !== '' && $mode === 'subscribe' && hash_equals($verifyToken, $token)) { http_response_code(200); echo $challenge; exit; }
    http_response_code(403); echo 'Verification failed'; exit;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $appSecret = trim((string)(getenv('META_APP_SECRET') ?: ''));
    if ($appSecret === '') { http_response_code(503); echo 'Webhook signature verification not configured'; exit; }
    $raw = file_get_contents('php://input') ?: '';
    $signature = (string)($_SERVER['HTTP_X_HUB_SIGNATURE_256'] ?? '');
    $expected = 'sha256=' . hash_hmac('sha256', $raw, $appSecret);
    if ($signature === '' || !hash_equals($expected, $signature)) { http_response_code(403); echo 'Invalid signature'; exit; }
    $decoded = json_decode($raw, true);
    if (!is_array($decoded)) { http_response_code(400); echo 'Invalid JSON'; exit; }
    rainbow_whatsapp_process_webhook($decoded);
    http_response_code(200); echo 'EVENT_RECEIVED'; exit;
}
