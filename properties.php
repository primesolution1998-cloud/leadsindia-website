...
<meta name="description" content="Search verified properties for sale and rent across India. Explore residential, commercial, plots and projects, or list your property free on Leads India.">
...
<section class="relative bg-ink text-white overflow-hidden">
  ...
  <div class="relative max-w-7xl mx-auto px-4 lg:px-6 pt-10 pb-16 lg:pt-16 lg:pb-20">
    <div class="max-w-3xl"><span class="inline-flex px-3 py-1 rounded-full border border-emerald-400/30 bg-emerald-400/10 text-emerald-300 text-xs font-black">VERIFIED PROPERTY MARKETPLACE</span><h1 class="mt-4 text-3xl sm:text-5xl font-black tracking-tight">Find the right property, faster.</h1><p class="mt-3 text-slate-300 max-w-2xl">Buy, rent, explore commercial spaces, plots and projects. Search live verified listings published through Leads India.</p></div>
...
<main class="max-w-7xl mx-auto px-4 lg:px-6 py-10">
  <div class="grid lg:grid-cols-[1fr_310px] gap-8 items-start">
    <section>
      ...
      <div id="empty" class="hidden rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center"><div class="text-4xl">🏙️</div><h3 class="font-black text-xl mt-3">No matching live properties yet</h3><p class="text-slate-500 text-sm mt-2">Try another city/filter or check again after new listings are verified.</p><a href="/list-property" class="inline-flex mt-5 px-5 py-3 rounded-xl bg-emerald-600 text-white font-black">List Property Free</a></div>
    </section>
    ...
    <aside class="space-y-5 lg:sticky lg:top-24">
      ...
      <div class="rounded-3xl bg-gradient-to-br from-emerald-600 to-teal-500 text-white p-5 shadow-lg"><span class="text-xs font-black uppercase">Property Owner?</span><h3 class="text-2xl font-black mt-2">Sell or rent faster</h3><p class="text-sm text-emerald-50 mt-2">Create an owner account, upload photos and submit your property for verification.</p><a href="/list-property" class="mt-4 block text-center rounded-xl bg-white text-emerald-700 font-black py-3">FREE List Your Property</a></div>
    </aside>
  </div>
  ...
  <section class="mt-14 grid lg:grid-cols-2 gap-6">
    <div class="rounded-3xl bg-white border border-slate-200 p-6"><span class="text-xs font-black text-emerald-600 uppercase">Price Insights</span><h2 class="text-2xl font-black mt-2">Locality price snapshot</h2><p class="text-slate-500 text-sm mt-2">Calculated from currently live Leads India listings. It updates automatically as verified inventory grows.</p><div id="insights" class="mt-5 space-y-3"></div></div>
    <div class="rounded-3xl bg-ink text-white p-6"><span class="text-xs font-black text-cyan-400 uppercase">Buyer Tools</span><h2 class="text-2xl font-black mt-2">Shortlist smarter</h2><div class="mt-5 grid sm:grid-cols-2 gap-3 text-sm"><div class="rounded-2xl bg-white/5 border border-white/10 p-4"><div class="text-xl">♡</div><div class="font-bold mt-2">Save favourites</div><p class="text-slate-400 text-xs mt-1">Shortlist properties in your browser.</p></div><div class="rounded-2xl bg-white/5 border border-white/10 p-4"><div class="text-xl">🕘</div><div class="font-bold mt-2">Recent activity</div><p class="text-slate-400 text-xs mt-1">Keep track of properties you viewed.</p></div><div class="rounded-2xl bg-white/5 border border-white/10 p-4"><div class="text-xl">🎙️</div><div class="font-bold mt-2">Voice search</div><p class="text-slate-400 text-xs mt-1">Search by voice on supported browsers.</p></div><div class="rounded-2xl bg-white/5 border border-white/10 p-4"><div class="text-xl">✓</div><div class="font-bold mt-2">Verified publishing</div><p class="text-slate-400 text-xs mt-1">Only LIVE-approved submissions are shown.</p></div></div></div>
  </section>
</main>

<footer class="mt-10 bg-ink text-slate-400 border-t border-white/10"><div class="max-w-7xl mx-auto px-4 lg:px-6 py-8 flex flex-col sm:flex-row gap-4 justify-between text-sm"><div><div class="text-white font-black">Leads India Properties</div><div class="text-xs mt-1">Verified property marketplace • India</div></div><div class="flex flex-wrap gap-4"><a href="/">Home</a><a href="/properties">Properties</a><a href="/search-property">Search</a><a href="/list-property">List Property Free</a></div></div></footer>
...
function card(p){const id=p.reference_id,img=(p.photos&&p.photos[0])||'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=900&auto=format&fit=crop&q=80',saved=fav.has(id),title=p.project_name||`${p.configuration||p.type||'Property'} in ${p.locality||p.city||'India'}`;return `<article class="rounded-3xl bg-white border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg transition"><div class="relative h-48 bg-slate-100"><img src="${img}" alt="${title.replace(/"/g,'')}" class="w-full h-full object-cover" loading="lazy"><span class="absolute top-3 left-3 rounded-full bg-emerald-600 text-white text-[10px] font-black px-2.5 py-1">✓ VERIFIED</span><button onclick="toggleFav('${id}')" class="absolute top-3 right-3 w-9 h-9 rounded-full bg-white/95 shadow font-black text-lg">${saved?'♥':'♡'}</button></div><div class="p-5"><div class="text-xs text-slate-500">📍 ${p.locality||''}${p.locality&&p.city?', ':''}${p.city||''}</div><h3 class="font-black mt-1 line-clamp-2">${title}</h3><div class="mt-3 flex items-end justify-between gap-3"><div><div class="text-xl font-black">${money(p.price)}</div><div class="text-xs text-slate-500 mt-1">${p.configuration||p.type||''} ${p.area?'• '+p.area:''}</div></div><span class="text-xs font-bold text-blue-600">${p.purpose||''}</span></div><button onclick="viewProperty('${id}')" class="mt-4 w-full rounded-xl bg-slate-950 text-white py-2.5 text-sm font-black">View Property</button></div></article>`}
...
function viewProperty(id){recent=[id,...recent.filter(x=>x!==id)].slice(0,20);localStorage.setItem('li_property_recent',JSON.stringify(recent));updateActivity();const p=all.find(x=>x.reference_id===id);if(p){alert(`${p.project_name||p.type||'Property'}\n${p.locality||''}, ${p.city||''}\n${money(p.price)}\n\nReference: ${p.reference_id}`)}}
window.viewProperty=viewProperty;
function updateActivity(){$('favCount').textContent=fav.size;$('recentCount').textContent=recent.length}
function renderProjects(){const projects=all.filter(p=>p.project_name).slice(0,6);$('projectGrid').innerHTML=projects.length?projects.map(p=>`<div class="rounded-3xl bg-white border border-slate-200 p-5"><div class="flex items-center justify-between"><span class="text-[10px] font-black px-2 py-1 rounded bg-blue-50 text-blue-700">PROJECT</span>${p.verified?'<span class="text-[10px] font-black text-emerald-600">VERIFIED</span>':''}</div><h3 class="font-black text-lg mt-3">${p.project_name}</h3><p class="text-sm text-slate-500 mt-1">${p.locality||''}, ${p.city||''}</p><div class="mt-4 font-black">${money(p.price)}</div><div class="text-xs text-slate-500 mt-1">${p.configuration||p.type||''}</div></div>`).join(''):`<div class="md:col-span-2 lg:col-span-3 rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">Verified projects will appear here as they are approved.</div>`}
