...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($confirm1 !== 'yes' || $confirm2 !== 'yes' || $confirm3 !== 'DELETE' || $confirm4 !== $ref) {
        ...
        li_audit($record, $from, 'OWNER_DELETED', 'OWNER', 'Hidden from owner dashboard; backend record and photos retained.');
...
$p = $record['property'] ?? [];
?>
<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Delete Property | Leads India</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-slate-950 text-white min-h-screen"><main class="max-w-xl mx-auto px-4 py-10"><a href="/owner/dashboard.php" class="text-emerald-400">← Back to My Properties</a><div class="mt-5 p-6 rounded-2xl bg-slate-900 border border-red-500/30"><h1 class="text-2xl font-black text-red-300">Delete Property</h1><p class="text-slate-400 mt-2">This will remove the property from your dashboard and public listing. Backend data and photos will be retained securely for records.</p><div class="mt-4 p-3 bg-white/5 rounded-xl"><b><?=htmlspecialchars($ref)?></b><br><span class="text-sm text-slate-400"><?=htmlspecialchars(($p['configuration']??'Property').' • '.($p['locality']??'').' • '.($p['city']??''))?></span></div><?php if($error): ?><div class="mt-4 p-3 bg-red-500/10 border border-red-500/30 rounded-xl text-red-200"><?=htmlspecialchars($error)?></div><?php endif; ?><form method="post" class="mt-5 space-y-4"><input type="hidden" name="csrf" value="<?=htmlspecialchars(owner_csrf_token())?>"><input type="hidden" name="reference_id" value="<?=htmlspecialchars($ref)?>"><label class="block p-3 border border-white/10 rounded-xl"><input required type="checkbox" name="confirm1" value="yes" class="mr-2">1. I selected the correct property.</label><label class="block p-3 border border-white/10 rounded-xl"><input required type="checkbox" name="confirm2" value="yes" class="mr-2">2. I understand this property will disappear from my dashboard and public listing.</label><label class="block"><span class="text-sm font-bold">3. Type DELETE</span><input required name="confirm3" autocomplete="off" class="mt-1 w-full bg-slate-950 border border-white/20 rounded-xl p-3" placeholder="DELETE"></label><label class="block"><span class="text-sm font-bold">4. Type Property ID: <?=htmlspecialchars($ref)?></span><input required name="confirm4" autocomplete="off" class="mt-1 w-full bg-slate-950 border border-white/20 rounded-xl p-3" placeholder="<?=htmlspecialchars($ref)?>"></label><button type="submit" class="w-full py-3 rounded-xl bg-red-600 hover:bg-red-500 font-black" onclick="return confirm('FINAL CONFIRMATION: Remove this property from your account view?');">Delete Property</button></form></div></main></body></html>
